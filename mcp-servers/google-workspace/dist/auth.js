...
exports.GoogleWorkspaceConfigurationError = exports.DEFAULT_GOOGLE_REDIRECT_URI = void 0;
exports.readGoogleOAuthEnv = readGoogleOAuthEnv;
...
exports.getFriendlyGoogleErrorMessage = getFriendlyGoogleErrorMessage;
const { google } = require('googleapis');
const types_1 = require("./types");
exports.DEFAULT_GOOGLE_REDIRECT_URI = 'http://127.0.0.1:3000/oauth2callback';
class GoogleWorkspaceConfigurationError extends Error {
    constructor(message) {
        super(message);
        this.name = 'GoogleWorkspaceConfigurationError';
    }
}
exports.GoogleWorkspaceConfigurationError = GoogleWorkspaceConfigurationError;
function readGoogleOAuthEnv(env = process.env) {
    const clientId = env.GOOGLE_CLIENT_ID?.trim();
    const clientSecret = env.GOOGLE_CLIENT_SECRET?.trim();
    const refreshToken = env.GOOGLE_REFRESH_TOKEN?.trim();
    if (!clientId || !clientSecret || !refreshToken) {
        throw new GoogleWorkspaceConfigurationError('Google Workspace is not connected yet. Add GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, and GOOGLE_REFRESH_TOKEN to your environment, then try again.');
    }
    return {
        clientId,
        clientSecret,
...
function createOAuthClient(env, redirectUri = exports.DEFAULT_GOOGLE_REDIRECT_URI) {
    return new google.auth.OAuth2(env.clientId, env.clientSecret, redirectUri);
}
function generateConsentUrl(redirectUri = exports.DEFAULT_GOOGLE_REDIRECT_URI) {
    ...
    return oauthClient.generateAuthUrl({
        access_type: 'offline',
        prompt: 'consent',
        scope: [...types_1.GOOGLE_WORKSPACE_SCOPES],
    });
}
async function exchangeCodeForRefreshToken(code, redirectUri = exports.DEFAULT_GOOGLE_REDIRECT_URI) {
    const env = readGoogleOAuthEnv({
        ...process.env,
        GOOGLE_REFRESH_TOKEN: process.env.GOOGLE_REFRESH_TOKEN || 'placeholder-refresh-token',
    });
    ...
    if (!tokens.refresh_token) {
        throw new GoogleWorkspaceConfigurationError('Google did not return a refresh token. Run the consent step again and make sure Google shows the permission screen.');
    }
...
function getFriendlyGoogleErrorMessage(error) {
...
async function runCli() {
    ...
    if (command === 'exchange-code') {
        if (!value) {
            throw new GoogleWorkspaceConfigurationError('Paste the code from the browser URL after the exchange-code command.');
        }
    ...
    console.log([
        ...
        '',
        `Default redirect URI: ${exports.DEFAULT_GOOGLE_REDIRECT_URI}`,
    ].join('\n'));
}
