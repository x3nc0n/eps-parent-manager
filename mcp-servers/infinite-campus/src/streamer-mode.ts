...
export function maskStudentProfile(profile: StudentProfile): StudentProfile {
  if (!isStreamerModeEnabled()) return profile;
...
export function maskGradeEntry(entry: GradeEntry): GradeEntry {
  if (!isStreamerModeEnabled()) return entry;
...
export function maskScheduleEntry(entry: ScheduleEntry): ScheduleEntry {
  if (!isStreamerModeEnabled()) return entry;
  return {
    ...entry,
    teacherName: maskFullName(entry.teacherName),
...
export function maskAssignmentRecord(record: AssignmentRecord): AssignmentRecord {
  if (!isStreamerModeEnabled()) return record;
...
export function maskReportCardCourse(course: ReportCardCourse): ReportCardCourse {
  if (!isStreamerModeEnabled()) return course;
  return {
    ...course,
...
export function maskGradesSnapshot(snapshot: GradesSnapshot): GradesSnapshot {
  if (!isStreamerModeEnabled()) return snapshot;
  return {
    ...snapshot,
...
export function maskAttendanceRecord(record: AttendanceRecord): AttendanceRecord {
  if (!isStreamerModeEnabled()) return record;
  return { ...record, raw: undefined };
}

export function maskAttendanceSnapshot(snapshot: AttendanceSnapshot): AttendanceSnapshot {
  if (!isStreamerModeEnabled()) return snapshot;
  return {
    ...snapshot,
    student: maskStudentProfile(snapshot.student),
...
export function maskScheduleSnapshot(snapshot: ScheduleSnapshot): ScheduleSnapshot {
  if (!isStreamerModeEnabled()) return snapshot;
  return {
    ...snapshot,
    student: maskStudentProfile(snapshot.student),
...
export function maskAssignmentsSnapshot(snapshot: AssignmentsSnapshot): AssignmentsSnapshot {
  if (!isStreamerModeEnabled()) return snapshot;
  return {
    ...snapshot,
    student: maskStudentProfile(snapshot.student),
...
export function maskReportCardSnapshot(snapshot: ReportCardSnapshot): ReportCardSnapshot {
  if (!isStreamerModeEnabled()) return snapshot;
